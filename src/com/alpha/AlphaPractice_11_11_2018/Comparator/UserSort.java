    ...
    public static HashSet<String> searchForEqualsUsers(ArrayList<User> arrayList, ArrayList<User> arrayList1){
        ...
        for (int i = 0; i < arrayList.size(); i++) {
            ...
            for (int j = 0; j < arrayList1.size(); j++) {
                String user1 = arrayList1.get(j).getUsername()
                        + " " + arrayList1.get(j).getEmail() + " "
                        + Arrays.toString(arrayList1.get(j).getPasswordHash());
                if (user.compareTo(user1)==0) result.add(user);
            }
        }
        return result;
    }
